...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public void updateWalletInBase(String isim, String soyisim, String kartNo, String yeniISim,String yeniSoyisim, String yeniKartNo) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.beginTransaction();

        try {
            ContentValues cv = new ContentValues();
            cv.put(CARD_NO_KOLON_ISMI, yeniKartNo);
            cv.put(CARD_PERSON_NAME_KOLON_ISMI, yeniISim);
            cv.put(CARD_PERSON_SURNAME_KOLON_ISMI, yeniSoyisim);
            //cv dekiler yeniler olacak

            String whereClause = CARD_PERSON_NAME_KOLON_ISMI + " = ? AND " + CARD_PERSON_SURNAME_KOLON_ISMI + " = ? AND " + CARD_NO_KOLON_ISMI + " = ?";

            String[] whereArgs = {isim, soyisim, kartNo};
            //bu argümanlar ise eskiler olacak
            ...
            if (affectedRows > 0) {
                ...
                // Perform RecyclerView update here
            } else {
